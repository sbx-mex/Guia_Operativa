(() => {
  ...
  function focusHeading(view) {
    const target = view === "training" ? $("trainingHeading") : view === "search" ? document.querySelector("#searchView h1") : document.querySelector("#homeView h1");
  ...
  function showView(view, options = {}) {
    ...
    if (options.focus !== false) requestAnimationFrame(()=>focusHeading(view));
  }
  function trail() {
    const parts = [];
  ...
  document.querySelectorAll("[data-view]").forEach(button=>button.onclick=()=>showView(button.dataset.view));document.querySelectorAll("[data-close]").forEach(button=>button.onclick=()=>$(button.dataset.close).close());$("homeButton").onclick=()=>showView("home");$("recipeSearch").oninput=event=>{state.query=event.target.value;state.visible=12;renderSearch();};$("clearSearch").onclick=()=>{state.query="";$("recipeSearch").value="";renderSearch();$("recipeSearch").focus();};$("showMore").onclick=()=>{state.visible+=12;renderSearch();};$("resumeTraining").onclick=resumeTraining;$("homeBeverageCount").textContent=cms.contents.filter(item=>item.category!=="Alimentos").length;$("homeFoodCount").textContent=cms.contents.filter(item=>item.category==="Alimentos").length;addEventListener("keydown",event=>{if(state.view==="search"&&event.key==="/"&&document.activeElement!==$("recipeSearch")){event.preventDefault();$("recipeSearch").focus();}if(state.view==="training"&&state.route&&!event.altKey&&!event.ctrlKey&&!event.metaKey&&!["INPUT","BUTTON","SUMMARY"].includes(document.activeElement?.tagName)){if(event.key==="ArrowLeft")moveStep(-1);if(event.key==="ArrowRight")moveStep(1);}});addEventListener("popstate",()=>showView(location.hash==="#practicar"?"training":location.hash==="#biblioteca"?"search":"home",{history:false}));if("serviceWorker" in navigator)addEventListener("load",()=>navigator.serviceWorker.register("sw.js").catch(()=>{}));updateResume();setupInstall();showView(location.hash==="#practicar"?"training":location.hash==="#biblioteca"?"search":"home",{history:false,focus:false});
})();
